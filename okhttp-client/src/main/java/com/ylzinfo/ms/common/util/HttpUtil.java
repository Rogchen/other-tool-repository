...
import com.ylzinfo.dto.util.TypeUtil;
import okhttp3.*;
...
public class HttpUtil {
	...
	private static int CONNECT_TIME_OUT = 50; // 连接超时，默认5s
	private static int READ_TIME_OUT = 50;// 读取超时，默认5s
	private static int WRITE_TIME_OUT = 50;// 写入超时，默认5s
...
* 发送post请求，并将返回结果解析成对应对象
		...
		public Result postForResult() throws IOException {
			return postForObject(CommonResult.class);
		}
...
* 发送get请求，并将返回结果解析成结果对象
		...
		public Result getForResult() throws IOException {
			return getForObject(CommonResult.class);
		}
